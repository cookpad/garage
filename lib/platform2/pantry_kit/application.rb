...
module Platform2
  module PantryKit
    ...
    class Application
      ...
      def collect_docs!
        scan_files(Dir.glob("app/controllers/platform2/*.rb"))
      end
      ...
      def docs
        Dir.glob("doc/platform2/resources/*.md").sort.map {|f| Documentation.new(f) }
      end
      ...
      def routes_with_docs
        all_routes = @app.routes.routes.select {|r| r.path.spec.to_s.start_with?('/platform2/')}
    ...
    class Documentation
      ...
      def resource
        path.basename.to_s.sub(/\.md$/, '').camelize
      end
